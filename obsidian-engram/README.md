...
- **Review cadence pill.** A neutral pill shows two *medians* across a folder/note as `due | interval` (e.g. `~4d | 10d`): **left = median next-review time** (urgency — `~4d`, `now` when due/overdue; drifts toward `now` as you fall behind), **right = median interval** (maturity — the branch's characteristic spacing; `new` until cards mature). Median, not mean — spaced-repetition intervals are right-skewed, so a mean would be dragged into the future by a few mature cards and hide a near-term load. Together they let you rank branches by ripeness for a memory-palace walk and see how settled each branch is. Toggle it off under Settings → *Show review cadence*.
