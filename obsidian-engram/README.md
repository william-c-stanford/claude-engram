...
- **Review cadence pill.** A neutral pill shows the *median* next-review time across a folder/note (`~4d`, `~3w`, `now`) so you can gauge and manage review frequency across the tree. Median, not mean — spaced-repetition intervals are right-skewed, so a mean would be dragged into the future by a few mature cards and hide a near-term load. `now` means the branch is due or overdue. Toggle it off under Settings → *Show review cadence*.
